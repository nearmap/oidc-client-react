...
export default class Oidc extends React.Component {
  setupEventDispatchers() {
    ...
    events.addSilentRenewError(err => this.handle(this.props.onSilentRenewError, err)); // TODO: should we automatically signin in like we do with the workaround?
  ...
  handleUserLoaded(user) {
    ...
    if (state !== undefined) {
      ...
      const url = typeof this.props.stateToUrl === 'function' ? this.props.stateToUrl(state) : `${location.pathname}${state}`;
...
_defineProperty(Oidc, "propTypes", {
